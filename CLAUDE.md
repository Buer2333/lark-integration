...
| 2026-08-16 | 新增 `h3-tunnel.service`（VPS→智川云 ah01 的 8188 隧道，密钥 `~/.ssh/h3_tunnel_ed25519`，实例侧 authorized_keys 由 `/opt/start.sh` key-reseed 段保证重启存活）；首次 `sudo systemctl enable --now h3-tunnel`（deploy-systemd.yml 只装文件不 enable 新 service） | lark-bot webapp 启用 MiniMax H3 provider（生产），端点 = 隧道后的 127.0.0.1:8188 |
| 2026-09-19 11:49 UTC | 手工给 7 个报表 unit 加 drop-in `50-noswap.conf`（`MemorySwapMax=0`），机型 e2-micro→e2-small | 整机 OOM 僵死（用户态无响应 30 min，硬 reset）。**只堵了 swap 没抬 MemoryMax**，09-19~20 四个 unit 静默 OOM 39 次——见 lark-bot docs/discipline/vps-deployment.md「内存上限判读」 |
| 2026-09-20 16:16 UTC | 合并 #10 后 `sudo rm` 7 个 `50-noswap.conf` + `rmdir *.service.d` + `daemon-reload` | `MemorySwapMax=0` 已内联进 deploy/*.service，drop-in 变 drift 源；deploy-systemd.yml 不管理 service.d |
| 2026-09-20 16:20 UTC | `systemd-run --unit=oom-selftest -p MemoryMax=20M -p MemorySwapMax=0 -p OnFailure=unit-failure-alert@oom-selftest.service` 自杀测试；`reset-failed` team-lead-report / morning-briefing / editor-audit-report | 验证 OnFailure→Inbox 告警链路真出卡（`inbox sent`）；清掉修复前残留的 failed 状态，让下一次失败干净冒出 |
